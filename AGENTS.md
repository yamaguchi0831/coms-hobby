...
## コンポーネント制作フロー（スクリーンショットを受け取った場合）

- スクリーンショットを受け取り、このデザインを再現して、という意味のメッセージを受けた後、実装前に見えている構成・文言・要素数・レイアウトを文字起こしし、ユーザー確認を取ってからHTML/CSS作成に進む。
- スクリーンショットからコンポーネントを作成する場合は、まず `temp` フォルダ内に確認用HTMLを作成する。
- `<main>`は使用しない
- 共通cssへリンクを貼る
- コンポーネント用CSSは、htmlのhead内`<style>`に記述する。
- コーディング後は、下記「デザイン・CSS準拠レビュー」を行ってからユーザー確認へ進む。
...
- コンポーネント用CSSがHTMLのhead内`<style>`にまとまっているか。
...
- Codex側でブラウザ確認を行う場合は、`file://` ではなくローカルサーバー経由を基本とする。確認前に `http://127.0.0.1:8123/` が応答するか確認し、停止している場合はプロジェクト直下をルートにして `python -m http.server 8123 --bind 127.0.0.1` を常駐起動してから確認する。
- `temp` フォルダ内の確認用HTMLは `http://127.0.0.1:8123/temp/対象ファイル.html` で開く。`ERR_CONNECTION_REFUSED` が出た場合は、HTMLやCSSの問題ではなくローカルサーバー停止を最初に疑い、サーバーを起動し直してから再読み込みする。
- Codex側のブラウザ自動チェックは必須工程にしない。`file://`、`localhost`、日本語パス、Google Drive配下のパスでブロックや確認遅延が起きる場合は、無理に継続せず静的確認に留める。
- Codex側では必要に応じて、HTML/CSS構文、クラス名、画像パス、リンクパス、モバイル横幅のはみ出しリスクを補助的に確認する。
- ハンバーガーメニュー、矢印、開閉、表示切り替えなど状態があるUIは、必要な各状態を確認する。
- レスポンシブ時に横並びから縦並びへ変化するUIでは、矢印や余白、CTA位置などの見え方も確認する。
